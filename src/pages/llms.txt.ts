...
/**
 * Endpoint dynamique : genere /llms.txt a chaque build.
 * Source: src/data/articles.ts.
 * Standard: llmstxt.org (Jeremy Howard, 2024).
 *
 * Format markdown lu par ChatGPT Search, Claude, Perplexity et autres
 * moteurs de reponse pour decouvrir les contenus du site.
 */
...
`;
  ...
  // BOM UTF-8 en tete : force la detection de l'encodage par les navigateurs
  // meme si Apache ne sert pas le bon Content-Type (cas OVH mutualise sur .txt).
  // Le .htaccess force aussi le charset, mais le BOM est une ceinture de plus.
  const BOM = '﻿';
